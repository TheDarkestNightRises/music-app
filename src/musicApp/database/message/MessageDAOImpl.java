...
//  }
  ...
  @Override public ArrayList<Message> getLast40Messages()
  {
    try
    {
      ...
      while(resultSet.next())
      {
        ...
        LocalDate date = LocalDate.of(timestamp.getYear(), timestamp.getMonth(),
            timestamp.getDay());
        LocalTime time = LocalTime.of(timestamp.getHours(), timestamp.getMinutes(), timestamp.getSeconds(), timestamp.getNanos());
  ...
  @Override public void addMessage(User user, Message message)
  {
    try
    {
      ...
      PreparedStatement statement = connection.prepareStatement("INSERT INTO message(text, sent_on, username)"
          + "VALUES (?, ?, ?);");
      statement.setString(1,message.getMessage());
      Timestamp timestamp = new Timestamp(message.getDate().getYear(), message.getDate().getMonthValue(), message.getDate().getDayOfMonth(), message.getTime().getHour(), message.getTime().getMinute(), message.getTime().getSecond(), message.getTime().getNano());
